...
function StackMenu() {
  const { currentAccount, tokenOfOwnerBached, tokenOfOwner, earningInfo, startEarning, claim, unstake, getCooldown } =
  ...
  async function getstack(pageNumber) {
    ...
    if (AllStack === undefined) {
      ...
      earning[0] =
        Tokens[0].token !== 0 ? parseFloat(convertEarn(await earningInfo([Tokens[0].token]))).toFixed(4) : undefined;
      earning[1] =
        Tokens[1].token !== 0 ? parseFloat(convertEarn(await earningInfo([Tokens[1].token]))).toFixed(4) : undefined;
      earning[2] =
        Tokens[2].token !== 0 ? parseFloat(convertEarn(await earningInfo([Tokens[2].token]))).toFixed(4) : undefined;
      earning[3] =
        Tokens[3].token !== 0 ? parseFloat(convertEarn(await earningInfo([Tokens[3].token]))).toFixed(4) : undefined;
      earning[4] =
        Tokens[4].token !== 0 ? parseFloat(convertEarn(await earningInfo([Tokens[4].token]))).toFixed(4) : undefined;
                ...
                <>
                  ...
                  {BtnBottomSlider.map((btn, iterate) => (
                    <div
                      ...
                      onClick={() => handelAction(btn.text, item.token)}
                      style={{ width: btn.width }}
                      key={btn.key}
                    >
                      <img src={btn.img} alt="Blue" width="100%" height="100%" />
                      <span className="centered-axis-xy" style={{ width: "100%", textAlign: "center" }}>
                        {iterate === 0
                          ? item.cooldown > 0
                            ? convertHour(item.cooldown)
                            : item.celestialType === 2
                            ? "Start Deceiving"
                            : item.celestialType === 3
                            ? "Guide and Deceive"
                            : btn.text
                          : btn.text}
                      </span>
